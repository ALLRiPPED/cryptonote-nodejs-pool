    ...
    async.waterfall([
        ...
        function(blocks, callback){
            ...
            redisClient.multi(redisCommands).exec(function(error, replies){
                ...
                for (let i in blocks) {
                    let block = blocks[i];
                    block.workerScores = replies[2*i];
                    block.workerShares = replies[2*i + 1];
                }
                callback(null, blocks);
            });
        },
        ...
        function(blocks, callback){
            ...
            blocks.forEach(function(block){
                ...
                let totalShares = parseFloat(block.shares);

                for (let worker in block.workerShares) {
                    // COIN:worker_unlocked:worker contains, at score TIME, value:
                    //     HEIGHT:HASH:DIFF:TOTALREWARD:WORKERREWARD:WORKERSHARES:TOTALSHARES[:WORKERSCORE:TOTALSCORE]
                    // (the last two are only present if score != shares, i.e. under slush rewards)
                    let data = [block.height, block.hash, block.difficulty, 'orphan', 'orphan', block.workerShares[worker], totalShares];
                    if (totalScore != totalShares && worker in block.workerScores)
                        data.push(block.workerScores[worker], totalScore);
                    orphanCommands.push(['zadd', config.coin + ':worker_unlocked:' + worker, block.time, data.join(':')]);
            ...
            blocks.forEach(function(block){
                ...
                if (block.workerScores) {
                    ...
                    Object.keys(block.workerScores).forEach(function (worker) {
                        ...
                        //     HEIGHT:HASH:DIFF:TOTALREWARD:WORKERREWARD:WORKERSHARES:TOTALSHARES[:WORKERSCORE:TOTALSCORE]
                        // (the last two are only present if score != shares, i.e. under slush rewards)
                        let data = [block.height, block.hash, block.difficulty, block.reward, workerReward, block.workerShares[worker], totalShares];
                        if (totalScore != totalShares)
                            data.push(block.workerScores[worker], totalScore);
                        unlockedBlocksCommands.push(['zadd', config.coin + ':worker_unlocked:' + worker, block.time, data.join(':')]);

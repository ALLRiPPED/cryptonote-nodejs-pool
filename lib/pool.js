...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    if (blockCandidate){
        ...
        redisCommands.push(['hincrby', config.coin + ':workers:' + miner.login, 'blocksFound', 1]);
...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    let shareBuffer = utils.cnUtil.construct_block_blob(template, new Buffer(nonce, 'hex'), cnBlobType);
